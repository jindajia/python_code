...
def evaluate(args):
    model.eval()
    losses = []
    print('args.bach_size: {}, total eval dataloader: '.format(args.batch_size))
    # print('total data set size: {}'.format(len(eval_dataloader['train'])))
    for step, batch in enumerate(eval_dataloader):
        if step % 10 == 0: print('step: {} '.format(step))
        with torch.no_grad():
...
model = GPT2LMHeadModel.from_pretrained(args.model_ckpt)
